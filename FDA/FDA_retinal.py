...
def replace_color_gray(img, src_clr = [255],dst_clr = [0]):
    img_arr = np.asarray(img, dtype=np.double)
...
src_file = "D://Project_code//Generator_vessel//make_fake_vessel_code//fake_vessel_image_IOSTAR//"
tar_file = "D://Project_code//Generator_vessel//make_fake_vessel_code//IOSTAR_graycrop//train//"
Save_image = "./FDA_retinalIOSTAR_image/"
Save_label = "./FDA_retinalIOSTAR_label/"
...
for i in range(len(files_src)):
    ...
    for j in range(len(tarlist)):
        ...
        im_src = Image.open(vessel_image_path).convert('L')
        im_trg = Image.open(target_domain_path).convert('L')
        ...
        im_label = replace_color_gray(im_label)
        ...
        Image.fromarray((img_FDA).astype('uint8')).convert('L').save(save_image_path)
