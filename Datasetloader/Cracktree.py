...
class DatasetCrack_aug(data.Dataset):
    ...
    def __init__(self, benchmark, datapath, split, img_mode, img_size,supervised):
        ...
        if self.supervised=='supervised':
            if self.split == 'train':
                self.img_path = os.path.join(datapath, 'train','fake_grayvessel')
                self.background_path = os.path.join(datapath,'train', 'img')
                self.ann_path = os.path.join(datapath, 'train','fake_gtvessel')
    ...
    def __getitem__(self, index):
        ...
        img = (img-torch.mean(img))/(torch.std(img)+1e-6)
        if self.supervised=='supervised':
            batch = {
                'img_name': img_name,
                'img': img,
    ...
    def load_frame_aff(self, img_name):
        img = self.read_testimg(img_name)
    ...
    def read_testimg(self, img_name):
        # maybe png
        return Image.open(os.path.join(self.img_path, img_name)).convert('L')

    def read_testmask(self, img_name):
        img_name = img_name.split(".")[0]
        mask = np.array(Image.open(os.path.join(self.ann_path, img_name + ".bmp")).convert('L'))
        mask[mask == 0] = 0
        mask[mask == 255] = 1
        mask = torch.from_numpy(mask).float().unsqueeze(0)
        return mask
