...
class DatasetDRIVE_aug(data.Dataset):
    def __init__(self, benchmark, datapath, split, img_mode, img_size,supervised):
        ...
        if self.supervised=='supervised':
            if self.split == 'train':
                self.img_path = os.path.join(datapath, 'train','fake_onlythin_vessel_gray')#fake vessel gray img
                self.background_path = os.path.join(datapath,'train', 'img')#target domain img
                self.ann_path = os.path.join(datapath, 'train','fake_onlythin_gt')#fake vessel gt
    ...
    def __getitem__(self, index):
        ...
        if self.supervised=='supervised' and self.split == 'train':
            ...
            img, anno_mask, background_img, ignore_mask, org_img_size = self.load_frame_fakevessel_whole_center(img_name, background_name)
    ...
    def load_frame_fakevessel_whole_center(self,img_name,background_name):
        ...
        ignore_mask = self.read_ignore_mask(background_name)#PIL
        org_img_size = img.size
        img_crop,bakground_crop, anno_crop, ignore_crop = self.get_params_center_forvessel(img, background_img, anno_mask, ignore_mask, (self.img_size, self.img_size))
        ...
        ignore_crop = np.array(ignore_crop)
        ignore_crop[ignore_crop == 0] = 0
        ignore_crop[ignore_crop == 255] = 1
        ignore_crop = torch.from_numpy(ignore_crop).float().unsqueeze(0)
        return img_FDA_Image, anno_crop, bakground_crop, ignore_crop, org_img_size
    ...
    def load_frame_fakevessel_cutvessel(self,img_name,background_name):
        ...
        gt_arrray = np.array(anno_mask)
        #gt_arrray = (np.array(anno_mask)[0,:,:]-1)*(-1)
        #gt_src = np.expand_dims(gt_arrray,axis=2)
        #gt_src = gt_src.transpose((2, 0, 1))
    ...
    def read_ignore_mask_torch(self, img_name):
        mask_name = img_name.split(".")[0]
        mask = np.array(Image.open(os.path.join(self.ignore_path, mask_name) + '.png'))
        mask[mask == 0] = 0
        mask[mask == 255] = 1
        mask = torch.from_numpy(mask).float().unsqueeze(0)
        return mask
    ...
    def load_metadata_supervised(self):
        ...
        if self.split == 'train':
            meta_file = os.path.join(self.datapath, 'split', 'train_fakevessel_only.txt')
    ...
    def get_params(self, img, output_size):
        ...
        #print("crop_i_j_w_h",i,j,th,tw,w,h)
        return i, j, th, tw
    ...
    def get_params_center_forvessel(self, img, background_img, anno_mask, ignore_mask, output_size):
        def _get_image_size(img):
            if F._is_pil_image(img):
                return img.size
            elif isinstance(img, torch.Tensor) and img.dim() > 2:
        ...
        ignore_mask = transform(ignore_mask)

        return img_crop,background_img,anno_mask_crop,ignore_mask