...
class Engine(object):
    ...
    def save_and_link_bestcheckpoint(self, snapshot_dir, log_dir, log_dir_link, name=None):
        ensure_dir(snapshot_dir)
        if not osp.exists(log_dir_link):
            link_file(log_dir, log_dir_link)
        if name is None:
            current_epoch_checkpoint = osp.join(snapshot_dir, 'epoch-{}_bestF1.pth'.format(
                self.state.epoch))
        else:
            current_epoch_checkpoint = osp.join(snapshot_dir, '{}_bestF1.pth'.format(
                name))
