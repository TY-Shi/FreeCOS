...
def LIOT_example(img):
	...
	gray_img = img  # convert to gray; if not retinal dataset, you can use standard grayscale api
...
Gray_image = Image.open("src_in_tar_vessel.png").convert('L')
Deal_img = distance_weight_binary_pattern_faster(Gray_image)
deal_filename = "LIOT_vessel.tiff"
print("Deal_img",Deal_img.shape)
im_r=Image.fromarray(Deal_img[:,:,0]).convert('L')
im_l=Image.fromarray(Deal_img[:,:,1]).convert('L')
im_u=Image.fromarray(Deal_img[:,:,2]).convert('L')
im_d=Image.fromarray(Deal_img[:,:,3]).convert('L')

im_r.save("LIOT_r.png")
im_l.save("LIOT_l.png")
im_u.save("LIOT_u.png")
im_d.save("LIOT_d.png")

im_trg = Image.open("./img/000_PPA_44_PSA_00_8.png").convert('L')
Deal_realimg = distance_weight_binary_pattern_faster(im_trg)
real_im_r=Image.fromarray(Deal_realimg[:,:,0]).convert('L')
real_im_l=Image.fromarray(Deal_realimg[:,:,1]).convert('L')
real_im_u=Image.fromarray(Deal_realimg[:,:,2]).convert('L')
real_im_d=Image.fromarray(Deal_realimg[:,:,3]).convert('L')

real_im_r.save("LIOT_real_r.png")
real_im_l.save("LIOT_real_l.png")
real_im_u.save("LIOT_real_u.png")
real_im_d.save("LIOT_real_d.png")
